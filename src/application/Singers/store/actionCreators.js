 ...
 * @returns
 */
export const getHotSingerList = (offset = 0) => (dispatch) => {
...
export const getMoreHotSingerList = () => (dispatch, getState) => {
 ...
 * @returns
 */
export const getSingerList = (type, area, alpha, offset = 0) => (dispatch) => {
...
export const getMoreSingerList = (type, area, alpha) => (dispatch, getState) => {
  const offset = getState().singers.getIn(['pageIndex']);
  const singerList = getState().singers.getIn(['singerList'])?.toJS();
  getSignerListRequest(type, area, alpha, offset).then((res) => {
    dispatch(updatePageIndex(offset + 1));
